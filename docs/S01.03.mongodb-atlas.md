...
You'll work on your local machine in the next session so you have time to install everything.

But today you work on the hosted version Atlas.

[MongoDB Atlas](https://www.mongodb.com/lp/cloud/atlas/try4-reg) provides Cloud-hosted service with both free and paid tiers.

Atlas gives us a hosted cluster on which we can create a database, import some sample collections and understand how to do CRUD operations in MongoDB. You do not need to install anything. for now.
...
The JSON that specifies the filtering arguments is called a **query predicate**. It is an expression that returns a boolean indicating whether a document matches the specified query. For example, { name: { $eq: "Alice" } }, which returns documents that have a field "name" whose value is the string "Alice".
...
10. find the number of films per genre
...
- To set up MongoDB Atlas, a cloud-hosted database service, including cluster creation and security configuration
