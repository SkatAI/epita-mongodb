...
- [NoSQL and graph databases](#nosql-and-graph-databases)
  ...
  - [NoSQL](#nosql)
    - [Hierarchy of NoSQL database:](#hierarchy-of-nosql-database)
...
Big pain, big hassle, headache, bugs, and extra costs ＄＄＄.
...
New Nutriscore version comes up, just transform the Nutriscore label into an dictionary with versions as keys:
...
Extra caution is required to avoid chaos and data inconsistencies. Query performance might be affected if changes in yje data structure  lead to inefficient or inconsistent indexing.
...
- Product ->  Vegetables -> Oragnb / not Bio
