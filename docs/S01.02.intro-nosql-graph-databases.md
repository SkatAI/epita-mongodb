...
<img src="./../img/evolution-of-databases.png" width='80%' style='display: block; margin: auto; ' alt= "Evolution of database from 1970 to 2024">
...
Its underlying database is the open food fatcs database https://world.openfoodfacts.org/
Open Food Facts is a food products database made by everyone, for everyone. with over 3.5 Million food products.

Check for instance the information for Nutella

https://uk.openfoodfacts.org/product/3017620422003/nutella

and the one for ... Baguette

https://world.openfoodfacts.org/product/3250393046940/baguette-constance-cereales-250g-la-campaniere
...
In fact as regulation evolves, new data must be slowly added while keeping old data.

![Nutriscore](./../img/nutri-score-f_1200x800.jpg)
...
Then Nutriscore changes. so you need new Nutriscore labels while keeping the old one.

But not all products implement new nutriscore. Some have dropped the labelling altogether.
...
so there's plenty of null values in that nutriscore_new_label column and null values are evil 👹👹👹.

or since a product can have multiple nutriscore labels, you introduce a nutriscore version
...
and then you have to change all your SQL queries in your code base.

Big pain, big hassle, headache

## Introducing Schema Flexibility:

aka dealing with **unknown unknowns** in a fast ever changing world.
...
And multiple representations can co exist in the same database.

- no nutriscore
- A single nutriscore as string
- embedded nutriscore key values
...
The key advantage of NoSQL over SQL databases is that it doesn't require strict normalization, which makes it particularly suitable when:
...
Note: this was generated by an LLM and the LLM naturally added some meaning, some information to the relations between tables.
...
Graph database on the other hand however are all about **relationships**.
